...
function checkCliImports(filePath: string, content: string): void {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (coreImportPattern.test(line) && !line.includes('bootstrap') && !line.includes('errors')) {
      violations.push({
        file: relativePath,
        line: i + 1,
...
function checkFileWatchUsage(filePath: string, content: string): void {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Check for fs.watch
    if (line.includes('fs.watch') && !line.trim().startsWith('//') && !line.trim().startsWith('*')) {
      violations.push({
        file: relativePath,
        line: i + 1,
        rule: 'no-fs-watch',
    ...
    if (line.includes('fs.watchFile') && !line.trim().startsWith('//') && !line.trim().startsWith('*')) {
      violations.push({
        file: relativePath,
        line: i + 1,
...
function checkServiceIsolation(filePath: string, content: string): void {
  ...
  if (!relativePath.startsWith('src' + path.sep + 'core' + path.sep + 'services')) {
    return;
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const serviceImportPattern = /import\s+.*\s+from\s+['"]\.\/(?:Analysis|Learning|Search)Service/;
    if (serviceImportPattern.test(line) && !relativePath.includes('DiagnosticService')) {
      violations.push({
        file: relativePath,
        line: i + 1,
        rule: 'no-service-coupling',
        message: 'Services should not import each other directly. Use DI container for orchestration.'
      });
    }
  }
}
