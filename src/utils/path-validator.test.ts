...
describe('PathValidator', () => {
    ...
    describe('validateProjectPath', () => {
        ...
        it('should throw error for non-existent paths', () => {
            ...
            expect(() => PathValidator.validateProjectPath(nonExistent, 'test')).toThrow();
        });
        ...
        it('should include context in error message', () => {
            const nonExistent = join(testDir, 'non-existent');
            expect(() => PathValidator.validateProjectPath(nonExistent, 'TestTool')).toThrow(/TestTool/);
        });
    });
    ...
    describe('looksLikeProjectRoot', () => {
        ...
        it('should handle errors gracefully', () => {
            ...
            expect(PathValidator.looksLikeProjectRoot('/non/existent/path')).toBe(false);
        });
    });
    ...
    describe('isSafeProjectPath', () => {
        it('should allow paths within project root', () => {
            ...
            const result = PathValidator.isSafeProjectPath(subDir, testDir);
            expect(result).toBe(true);
        });

        it('should reject paths outside project root with ..', () => {
            const parentDir = join(testDir, '..', 'outside');
            const result = PathValidator.isSafeProjectPath(parentDir, testDir);
        ...
        it('should reject path traversal attempts', () => {
            const result = PathValidator.isSafeProjectPath('../../../etc/passwd', testDir);
            expect(result).toBe(false);
        });
        ...
        it('should handle nested relative paths', () => {
            const result = PathValidator.isSafeProjectPath('src/utils/helpers.ts', testDir);
            expect(result).toBe(true);
        });
        ...
        it('should reject null byte injection', () => {
            const result = PathValidator.isSafeProjectPath('src/file\x00.ts', testDir);
            // Path should be rejected or sanitized
            expect(result).toBe(false);
        });

        it('should handle Windows path separators', () => {
            const result = PathValidator.isSafeProjectPath('src\\utils\\file.ts', testDir);
    ...
    describe('ensureWithinProject', () => {
        it('should return path if within project', () => {
            ...
            const result = PathValidator.ensureWithinProject(subPath, testDir, 'test');
            expect(result).toBe(subPath);
        });

        it('should throw if path is outside project', () => {
            const outsidePath = join(testDir, '..', 'outside');
            expect(() => PathValidator.ensureWithinProject(outsidePath, testDir, 'test')).toThrow(/outside project/);
        });

        it('should throw if path does not exist', () => {
            const nonExistent = join(testDir, 'non-existent');
            expect(() => PathValidator.ensureWithinProject(nonExistent, testDir, 'test')).toThrow();
        });

        it('should include context in error message', () => {
            const nonExistent = join(testDir, 'non-existent');
            expect(() => PathValidator.ensureWithinProject(nonExistent, testDir, 'TestContext')).toThrow(/TestContext/);
        });
    });
