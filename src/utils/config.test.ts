...
import { SurrealBackendConfigAdapter } from '../storage/backend-unified.js';
...
describe('ConfigManager with Backend Abstraction', () => {
  ...
  describe('Vector Backend Configuration', () => {
    ...
    it('should reject non-SurrealDB backend configuration (single backend only)', () => {
      const configManager = ConfigManager.getInstance();
      ...
      // Should fail because only SurrealDB is supported
      expect(result.valid).toBe(false);
      expect(result.errors.length).toBeGreaterThan(0);
      expect(result.errors[0]).toContain('Only SurrealDB backend is supported');
      
      // Config should remain as surreal
  ...
  describe('Environment Variable Processing', () => {
    it('should ignore non-SurrealDB backend in environment variables', () => {
      // Set environment variables (should be ignored for non-surreal backends)
      ...
      const config = configManager.getVectorBackendConfig();
      // Should use surreal backend despite environment variable
      expect(config.type).toBe('surreal');
      // But should still pick up embedding config
  ...
  describe('Sanitized Configuration', () => {
    it('should provide sanitized configuration for logging (SurrealDB only)', () => {
      const configManager = ConfigManager.getInstance();
      
      // Try to set Qdrant configuration (should fail)
      ...
      // Should remain as surreal since Qdrant is not supported
      expect(sanitized.vectorBackend).toHaveProperty('type', 'surreal');
