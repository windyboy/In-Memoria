...
// Import unified MCP adapter (consolidated for code simplification)
import { UnifiedMCPAdapter } from "./adapters/unified-adapter.js";
...
export class CodeCartographerMCP {
    ...
    private unifiedAdapter!: UnifiedMCPAdapter;
    ...
    private initializeToolRegistry(): void {
        // Phase 3: Whitelisted MCP Tools Only
        // Unified Tools (Pure Adapters) - Whitelisted subset only
        this.toolRegistry.set("analyze_codebase", (args) =>
            this.unifiedAdapter.analyzeCodebase(args),
        );
        this.toolRegistry.set("search_codebase", (args) =>
            this.unifiedAdapter.searchCodebase(args),
        );
        this.toolRegistry.set("learn_codebase_intelligence", (args) =>
            this.unifiedAdapter.learnCodebaseIntelligence(args),
        );
        this.toolRegistry.set("get_semantic_insights", (args) =>
            this.unifiedAdapter.getSemanticInsights(args),
        );
        this.toolRegistry.set("get_pattern_recommendations", (args) =>
            this.unifiedAdapter.getPatternRecommendations(args),
        );
        this.toolRegistry.set("predict_coding_approach", (args) =>
            this.unifiedAdapter.predictCodingApproach(args),
        );
        this.toolRegistry.set("get_project_blueprint", (args) =>
            this.unifiedAdapter.getProjectBlueprint(args),
        );
        this.toolRegistry.set("get_intelligence_metrics", (args) =>
            this.unifiedAdapter.getIntelligenceMetrics(args),
        );
    }
    ...
    private async initializeComponents(): Promise<void> {
        try {
            ...
            // Initialize unified MCP adapter that uses services through DI Container
            // Phase 3: Consolidated for code simplification
            this.unifiedAdapter = new UnifiedMCPAdapter(this.container);
            Logger.info("Unified MCP adapter initialized (whitelisted tools only)");
    ...
    private setupHandlers(): void {
        // List available tools (Phase 3: Whitelisted tools only)
        this.server.setRequestHandler(ListToolsRequestSchema, async () => {
            return {
                tools: [
                    ...this.unifiedAdapter.tools,
                ],
            };
        });
     ...
     * Phase 3: Returns only whitelisted tools
     */
    getAllTools(): any[] {
        return [
            ...this.unifiedAdapter.tools,
