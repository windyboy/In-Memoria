...
export class SurrealVectorDB implements VectorStore {
    ...
    async initialize(collectionName: string = "in-memoria"): Promise<void> {
        try {
            ...
            const dbPath =
                process.env.IN_MEMORIA_VECTOR_DB_PATH ||
     ...
     */
    private async generateRealSemanticEmbedding(
        code: string,
    ): Promise<number[]> {
        // Check cache first
        ...
        Logger.debug(
            `⚙️ Generating new embedding for code (${code.length} chars)`,
        );
        // Use local embedding
        const embedding = await this.getLocalEmbedding(code);
        Logger.debug(
            `✅ Generated new embedding, dimension: ${embedding.length}`,
        );
