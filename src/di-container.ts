...
export function initializeDIContainer(projectPath?: string): DIContainer {
  ...
  container.registerFactory(ServiceKeys.DATABASE, () => {
    const dbPath = projectPath ? config.getDatabasePath(projectPath) : ':memory:';
  ...
  container.registerFactory(ServiceKeys.SEMANTIC_ENGINE, () => {
    ...
    return new SemanticEngine(database);
  });
  ...
  container.register(ServiceKeys.VECTOR_STORE, null); // Optional, set later if available
