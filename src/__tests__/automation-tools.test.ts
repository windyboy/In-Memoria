...
interface MockSemanticEngine {
  learnFromCodebase: Mock;
}
...
describe("AutomationTools", () => {
  ...
  describe("getLearningStatus", () => {
    it("should return correct status for empty database", async () => {
      const result = await automationTools.getLearningStatus({
        path: projectDir,
      });
  ...
  describe("autoLearnIfNeeded", () => {
    ...
    it("should handle learning errors gracefully", async () => {
      ...
      const mockDatabase = {
        ...
        getMigrator: vi
          .fn()
          .mockReturnValue({ getCurrentVersion: vi.fn().mockReturnValue(1) }),
      } as any;
  ...
  describe("quickSetup", () => {
    ...
    it("should include learning when not skipped", async () => {
      ...
      expect(
        result.steps.find(
          (s: { step: string; status: string }) => s.step === "learning",
        )?.status,
  ...
  describe("tools property", () => {
    it("should expose correct tool definitions", () => {
      ...
      expect(tools.map((t) => t.name)).toEqual(["auto_learn_if_needed"]);
